...
app.patch('/pokemon/:id', async (request, response, next) => {

  try {
    let id = request.params.id;
    let pokemonFromDb = await Pokemon.find({ _id: id});

    console.log(pokemonFromDb, id);
    if (!pokemonFromDb.length) {
      next('not found');
    } else {
      // {name: jacob} => ["name"]
      Object.keys(request.body).forEach(key => {
        pokemonFromDb[0][key] = request.body[key];
      });
      let savedPokemon = await pokemonFromDb[0].save();
      response.send(savedPokemon);
    }
...
app.put('/pokemon/:id', async (request, response, next) => {
  try {
    let id = request.params.id;
